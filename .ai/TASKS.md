...
| P6-001H-EVID | Capture approval-bound Qwen live evidence (promote staged synthetic reviewer artifact) | In progress | Live-invocation blocker diagnosed and fixed: Qwen Code CLI `0.19.2` returns the schema-valid review JSON in the result envelope's `result` **string** field, not `structured_output`. Parser fallback (`structured_output` → `structured_result` → JSON-parsed `result`) merged in PR #31 at `ff4dfad`; all existing schema/reviewer/task_id/budget checks and the fail-closed posture preserved; full suite 140 passing. Remaining: execute the bounded live run on the approved PC runner to produce `.bridge/P6-001H-EVID/` evidence, then `REVIEW_CLAUDE.json`. The approval-ledger entry `P6-001H-EVID-RUN-001` (RSK-1) commits with the evidence PR. |
