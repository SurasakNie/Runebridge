...
| P6-QWEN-ADAPTER-001 | Implement the Qwen reviewer adapter module and fake-CLI contract tests | Planned | Architect plan in `.bridge/P6-QWEN-ADAPTER-001/PLAN.md`; PC-only build — do not register adapter in `ENABLED_ADAPTERS` or activate in shared remote environment; CLI flags and envelope format must be confirmed during PC preflight |
