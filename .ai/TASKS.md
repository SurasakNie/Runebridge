...
| P6-001H | Decide Qwen provider and authentication path | Complete; live validated on approved PC runner | Provider/auth path recorded; shared remote environment returns egress-policy `403 Forbidden` to approved Qwen provider hosts, so live Qwen uses the approved `PC-first, VM-later` runner model |
