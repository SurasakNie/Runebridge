...
| Owner | Human + Codex |
| Status | Prep merged (PR #35, `6f4d48a`); Blocked on the PC run + per-run approval only |
| Branch | merged to `main`; PC run gated on the owner's approved runner |
...
| P6-001F | Execute bounded Codex validation | Blocked (prep merged in PR #35) | Parameters ratified 2026-06-28 (`P6-001F-RUN-001`, `codex-mini-latest`, `30 s`, `$0.06`, direct runner, local-only). Prep PR #35 (`6f4d48a`) **merged** — fixed the documented execution defects (removed the non-existent `workspace=` arg and phantom external workspace, corrected `ARTIFACT_ROOT` to `.bridge`, hardened `build_codex_adapter`'s prompt: create `fixture.txt`, `files_changed=["fixture.txt"]`, exact `a/`…`b/` diff headers, added `run_p6_001f.py`); validators unchanged, full suite 140 passing, no live call. Still requires: verified `codex --help` flags + auth + per-run approval on the PC runner. |
